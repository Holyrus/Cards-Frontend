...
const Profile = ({handleLogout, user, handleAccountDeleting}) => {
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center ${theme === 'Black' ? 'dark-scrollbar' : ''}`}>

      <div className={`flex-1 flex flex-col items-center w-full ${theme === 'Black' ? 'bg-[#0f1418]' : 'bg-[#f3fff2]'}`}>

        <div className="h-[43px] w-full bg-transparent">

        </div>

        <div className={`flex flex-col items-center justify-center h-[150px] w-[300px] sm:w-[540px]
           ${theme === 'Black' ? 'bg-[#0d0d0d]' : 'bg-white'}`}>
          <img src={Panda} alt="Panda" className="w-[60px]" />
          <h1 className={`font-bold ${theme === 'Black' ? 'text-white' : 'text-black'}`}>{user.name}</h1>
          <h2 className={`${theme === 'Black' ? 'text-white' : 'text-black'}`}>{user.username}</h2>
        </div>

        <div className={`flex flex-col items-start justify-start w-[300px] sm:w-[540px] mt-8
            ${theme === 'Black' ? 'bg-[#0d0d0d]' : 'bg-white'}`}>
          <button className={`text-[17px] pl-4 gap-2 border-b-1 h-[65px] w-full cursor-pointer flex flex-row justify-start items-center
            ${theme === 'Black' ? 'border-[#182026] text-white' : 'border-gray-200 text-black'}`} onClick={handleLogout}>
            <svg className={`w-[25px] ${theme === 'Black' ? 'text-white' : 'text-black'}`} fill="currentColor" focusable="false" aria-hidden="true" viewBox="0 0 24 24" data-testid="MeetingRoomIcon"><path d="M14 6v15H3v-2h2V3h9v1h5v15h2v2h-4V6zm-4 5v2h2v-2z"></path></svg>
            Logout
          </button>
        </div>
        ...
        <div className={`flex flex-col items-start justify-start w-[300px] sm:w-[540px] mt-8
            ${theme === 'Black' ? 'bg-[#0d0d0d]' : 'bg-white'}`}>
          <button className={`text-[#AAAAAA] text-[17px] pl-4 gap-2 border-b-1 h-[65px] w-full cursor-pointer flex flex-row justify-start items-center
            ${theme === 'Black' ? 'border-[#182026]' : 'border-gray-200'}`} onClick={handleAccountDeleting}>
      ...
      <div className={`fixed bottom-0 w-full flex-none flex flex-row items-center justify-center h-[55px]
        ...
        <Link to="/main" className={`flex flex-col items-center justify-start px-[60px] hover:cursor-pointer transition-all duration-75
          ...
          <svg width="35" height="35" viewBox="0 0 200 250" xmlns="http://www.w3.org/2000/svg">
            <g transform="rotate(10, 115, 130)">
              <rect className="text-gray-500" x="80" y="80" width="90" height="140" rx="10" ry="10" fill={`${theme === 'Black' ? '#0d0d0d' : 'white'}`} stroke="currentColor" strokeWidth="10"/>
            </g>
            <g transform="rotate(-10, 85, 100)">
              <rect className="text-gray-500" x="30" y="30" width="90" height="140" rx="10" ry="10" fill={`${theme === 'Black' ? '#0d0d0d' : 'white'}`} stroke="currentColor" strokeWidth="10"/>
            </g>
          </svg>
        ...
        <Link to="/profile" className={`flex flex-col items-center justify-start px-[60px] hover:cursor-pointer transition-all duration-75
          ...
          <svg width="35" height="35" aria-hidden="true" focusable="false" data-prefix="far" data-icon="user-vneck" className="svg-inline--fa fa-user-vneck" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 -100 448 650">
            <circle className={`${theme === 'Black' ? 'text-green-600' : 'text-black'}`} cx="120" cy="35" r="40" fill={`${theme === 'Black' ? 'black' : 'white'}`} stroke="currentColor" strokeWidth="40"/>
            <circle className={`${theme === 'Black' ? 'text-green-600' : 'text-black'}`} cx="328" cy="35" r="40" fill={`${theme === 'Black' ? 'black' : 'white'}`} stroke="currentColor" strokeWidth="40"/>
