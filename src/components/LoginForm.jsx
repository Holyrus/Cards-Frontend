    ...
    <div className={`min-h-screen flex flex-col items-center justify-between ${theme === 'Black' ? 'dark-scrollbar bg-[#0f1418]' : 'bg-white'}`}>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col items-center sm:items-start mb-10 sm:mb-50'>
        <div className={`relative flex mt-4 rounded-t-sm w-[280px] sm:w-[300px]
          ...
          <input value={username} onChange={handleUsernameChange} id="loginUsername" type="text" placeholder='' className={`z-10 peer w-full border-0 border-b-2 focus:border-green-500 focus:outline-none focus:ring-0 bg-transparent p-2 pt-4
           ${theme === 'Black' ? 'border-gray-600 text-gray-300' : 'border-gray-400 text-gray-900'}`} />
          <label htmlFor="loginUsername" className='z-5 absolute left-2 text-gray-500 text-[12px] peer-placeholder-shown:bottom-3.5 peer-placeholder-shown:text-[16px] peer-placeholder-shown:text-gray-400 peer-focus:top-0 peer-focus:text-green-500 peer-focus:text-[12px] transition-all'>Username</label>
        </div>


        <div className={`relative flex mt-4 rounded-t-sm w-[280px] sm:w-[300px]
          ${theme === 'Black' ? 'bg-[#0d0d0d]' : 'bg-white'}`}>
          <input value={password} onChange={handlePasswordChange} id="loginPassword" type={showPassword ? 'text' : 'password'} placeholder='' className={`z-10 peer w-full border-0 border-b-2 focus:border-green-500 focus:outline-none focus:ring-0 bg-transparent p-2 pt-4
           ${theme === 'Black' ? 'border-gray-600 text-gray-300' : 'border-gray-400 text-gray-900'}`} />
          <label htmlFor="loginPassword" className='z-5 absolute left-2 text-gray-500 text-[12px] peer-placeholder-shown:bottom-3.5 peer-placeholder-shown:text-[16px] peer-placeholder-shown:text-gray-400 peer-focus:top-0 peer-focus:text-green-500 peer-focus:text-[12px] transition-all'>Password</label>
          <button 
            type='button'
            onClick={(e) => {
              e.stopPropagation()
        ...
        <div className='flex flex-row justify-between w-[280px] sm:w-[300px] my-4'>
          ...
          <button type='submit' className={`rounded-full border-1 bg-green-700 border-green-700 font-semibold py-2 px-5 transition-all duration-300
