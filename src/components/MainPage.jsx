...
const MainPage = ({ decks, onDeckChange }) => {
  ...
  const updateFieldDeckMutation = useMutation({
    mutationFn: (updatedDeck) => decksService.updateField(updatedDeck.id, updatedDeck),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['decks'] })
    }
  })
  ...
  const mainDeckSetter = async (selectedDeck) => {
    try {
      ...
      .map(deck => {
          return updateFieldDeckMutation.mutateAsync({
            id: deck.id,
            mainDeck: false
          })
      })
      ...
      await updateFieldDeckMutation.mutateAsync({
        id: selectedDeck.id,
        mainDeck: true
      })
  ...
  const handleVoiceChange = (voice) => {
    updateFieldDeckMutation.mutate({id: currentDeck.id, voice: voice.name}, {
      onError: (error) => {
